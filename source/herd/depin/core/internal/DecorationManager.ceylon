...
shared class DecorationManager(Handlers handlers) {
	...
	shared Dependency|Dependency.Decoration decorate(Dependency dependency) {
		...
		Dependency|Dependency.Decoration decorated = decorators(dependency.declaration)
				.fold(dependency)((Dependency subject, Dependency.Decorator decorator) {
		...
		log.trace("Finished decorating ``dependency``");
		return decorated;
	}
