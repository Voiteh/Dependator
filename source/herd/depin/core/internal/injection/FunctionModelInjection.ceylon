...
shared class FunctionModelInjection(FunctionModel<> model, Dependency? container, {Dependency*} parameters) extends Injection(model, container, parameters) {
	shared actual Anything inject {
		...
		value resolvedContainer = if (exists container) then safe(() => container.resolve)((Throwable cause) => Error(cause, model, container, parameters))
		else null;
		log.trace("Resolved container: `` resolvedContainer else "null" `` for injecting into: ``model`` ");
		value resolvedParameters = safe(() => parameters.map((Dependency element) => element.resolve).select((Anything element) => !element is Defaulted))((Throwable cause) => Error(cause, model, container, parameters));
